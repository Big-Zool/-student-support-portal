...
var LongPoll = /* @__PURE__ */ __name(class {
  ...
  batchSend(messages2) {
    ...
    this.ajax("POST", { "Content-Type": "application/x-ndjson" }, messages2.join("\n"), () => this.onerror("timeout"), (resp) => {
      this.awaitingBatchAck = false;
...
conversations.post("/", async (c) => {
  ...
  const { data: thread, error: threadError } = await supabase.from("conversation_threads").insert({
...
conversations.get("/:threadId", async (c) => {
  ...
  const { data: thread, error: threadError } = await supabase.from("conversation_threads").select(`
  ...
  const { data: messages2, error: messagesError } = await supabase.from("conversation_messages").select(`
      id, body, created_at, sender_type,
      sender:sender_id (id, full_name, role)
    `).eq("thread_id", threadId).order("created_at", { ascending: true });
  if (messagesError)
    return c.json({ error: messagesError.message }, 500);
  return c.json({ thread, messages: messages2 });
});
conversations.patch("/:threadId/status", async (c) => {
  ...
  const { data: thread, error: fetchError } = await supabase.from("conversation_threads").select("assigned_to").eq("id", threadId).single();
  if (fetchError || !thread)
    return c.json({ error: "Thread not found" }, 404);
  if (profile.role === "sales" && thread.assigned_to !== profile.id) {
    return c.json({ error: "Forbidden" }, 403);
  }
  const { data, error } = await supabase.from("conversation_threads").update({ status, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", threadId).select().single();
  if (error)
    return c.json({ error: error.message }, 500);
  return c.json(data);
});
conversations.patch("/:threadId/assign", async (c) => {
  ...
  const { data, error } = await supabase.from("conversation_threads").update({ assigned_to: assignedTo, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", threadId).select().single();
  if (error)
    return c.json({ error: error.message }, 500);
  const { error: eventError } = await supabase.from("conversation_assignment_events").insert({
    thread_id: threadId,
...
messages.post("/:threadId/messages", async (c) => {
  ...
  const { data: thread, error: fetchError } = await supabase.from("conversation_threads").select("student_id, assigned_to").eq("id", threadId).single();
  if (fetchError || !thread)
    return c.json({ error: "Thread not found" }, 404);
  if (profile.role === "student" && thread.student_id !== profile.id) {
    return c.json({ error: "Forbidden" }, 403);
